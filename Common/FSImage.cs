...
public class FSImage
{
    ...
    /// <param name="filepath">原文件物理磁盘路径（水印、缩略图）或Save文件路径（按文字生成图片）</param>
    ...
    /// 生成缩略图
    /// </summary>
    /// <param name="newpath">缩略图的Save物理路径（包括文件名），如果为null或空则覆盖原图</param>
    ...
    private ImageFormat GetFormat()
    {
        if (_filepath == null || _filepath.IndexOf(".") < 0)
            throw new FileNotFoundException("文件路径不能为空或是文件名不correct!");

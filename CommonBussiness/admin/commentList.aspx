<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="commentList.aspx.cs" Inherits="CommonBusiness.admin.commentList" %>
            ...
            <ItemTemplate>
                <tr>
                 <td><asp:Label ID="lblId" runat="server" Text='<%#Eval("Id") %>' Visible="false"></asp:Label><asp:CheckBox ID="cbxAll" runat="server"   /></td>   
                    <td><%# GetObjTitle(Eval("productId")) %></td>
                     <td><%#GetImg(Eval("img1Url"), Eval("img2Url"), Eval("img3Url"), Eval("img4Url"))%></td>                
                    <td><%#Eval("commentInfo") %></td>
                    <td><%#Eval("addTime","{0:yyyy-MM-dd}") %></td>                   
                    <td><%#GetAddUser(Eval("addUser")) %></td>
                </tr>
            </ItemTemplate>
        </asp:Repeater>
        <tr><td class="tleft" style="background:#eee;" colspan="10"></td></tr>

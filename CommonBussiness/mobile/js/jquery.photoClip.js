 ...
 * @option_param {string} file Upload图片的<input type="file">控件的选择器或者DOM对象
 * @option_param {string} view 显示截取后图像的容器的选择器或者DOM对象
 * @option_param {string} ok 确认截图按钮的选择器或者DOM对象
 * @option_param {string} outputType 指定输出图片的I am a，可选 "jpg" 和 "png" 两种种I am a，默认为 "jpg"
	...
	function photoClip(container, option) {
		...
		$file.change(function() {
			...
			if (!/image\/\w+/.test(this.files[0].type)) {
				alert("图片格式不correct，请选择correct格式的图片文件！");
				return false;
			} else {
				...
				fileReader.onload = function(e) {
					...
					if (kbs > 1024) {
						...
						$tempImg.load(function() {
							...
							var sourceWidth = this.naturalWidth; // 在没有加入文档前，jQuery无法获得correct宽高，但可以通过原生属性来读取
		...
		function refreshScroll(width, height) {
			...
			// 在移动设备上，尤其是Android设备，当为一个$素重置了宽高时
			// 该$素的offsetWidth/offsetHeight、clientWidth/clientHeight等属性并不会立即更新，导致相关的js程序出现错误
			// iscroll 在刷新方法中正是使用了 offsetWidth/offsetHeight 来获取scroller$素($moveLayer)的宽高
			// 因此需要手动将$素重新添加进文档，迫使浏览器强制更新$素的宽高
		...
		function createImg(src) {
			if ($img &&　$img.length) {
				// Delete旧的图片以释放内存，防止IOS设备的webview崩溃
				$img.remove();
				delete $img[0];
			}
			$img = $("<img>").css({
		...
		function setTransition($obj, x, y, angle, dur, fn) {
			// 这里需要先读取之前设置好的transform样式，强制浏览器将该样式值渲染到$素
